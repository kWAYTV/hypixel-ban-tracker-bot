...
class BanTracker:
    def __init__(self) -> None:
        ...
        self.img_logo = "https://i.imgur.com/qXuAK9O.png"
    ...
    def check_bans(self):
        ...
        if self.owd_bans != None and self.ostaff_bans != None:
            ...
            if wban_dif > 0:
                ...
                ).set_author(name=f"Watchdog banned {wban_dif} player{'s'[:wban_dif^1]}!").set_thumbnail(url=self.img_logo).set_footer(text="Hypixel Ban Tracker | .gg/kws")
                embeds.append(embed)
            ...
            if sban_dif > 0:
                embed = discord.Embed(
                    ...
                    description=f"<t:{math.floor(time.time())}:R>",
                ).set_author(name=f"Staff banned {sban_dif} player{'s'[:sban_dif^1]}!").set_thumbnail(url=self.img_logo).set_footer(text="Hypixel Ban Tracker | .gg/kws")
                embeds.append(embed)
...
class BanTrackerBot(discord.Client):
    def __init__(self, intents: discord.Intents) -> None:
        ...
        async def changeStatus():
            await self.change_presence(status=discord.Status.do_not_disturb, activity=discord.Activity(type=discord.ActivityType.watching, name=next(status)))
